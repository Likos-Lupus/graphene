use super::{MAX_ENTRIES, MAX_NAME_BYTES, archive_error, deflate::inflate_raw};
...
pub(super) fn central_entries(bytes: &[u8]) -> Result<Vec<ZipEntry>> {
    let eocd =
        find_eocd(bytes).ok_or_else(|| archive_error("native archive has no valid end record"))?;
    if eocd + 22 > bytes.len() {
        return Err(archive_error("native archive end record is truncated"));
    }
    ...
    if disk != 0 || central_disk != 0 || disk_entries != total_entries {
        return Err(archive_error(
            "multi-disk native ZIP archives are unsupported",
        ));
    }
    ...
    {
        return Err(archive_error(
            "ZIP64 native archives are unsupported in Phase 1",
        ));
    }
    ...
    {
        return Err(archive_error(
            "native archive central directory is out of bounds",
        ));
    }
    ...
    for _ in 0..total_entries {
        if cursor + 46 > central_end || u32_at(bytes, cursor)? != 0x0201_4b50 {
            return Err(archive_error(
                "native archive central directory entry is malformed",
            ));
        }
        ...
        {
            return Err(archive_error("ZIP64 native archive entry is unsupported"));
        }
        ...
        let record_len = 46usize
            ...
            .ok_or_else(|| archive_error("native archive entry size overflowed"))?;
        if name_len == 0
        ...
        {
            return Err(archive_error(
                "native archive entry name or record is invalid",
            ));
        }

        if flags & 0x0001 != 0 {
            return Err(archive_error(
                "encrypted native archive entries are unsupported",
            ));
        }
        ...
        let name = std::str::from_utf8(name_bytes)
            .map_err(|source| {
                archive_error("native archive entry name is not UTF-8").with_source(source)
            })?
            .to_owned();
    ...
    if cursor != central_end {
        return Err(archive_error(
            "native archive central directory length is inconsistent",
        ));
    }
...
pub(super) fn extract_entry(
    ...
    if cursor + 30 > bytes.len() || u32_at(bytes, cursor)? != 0x0403_4b50 {
        return Err(archive_error("native archive local entry is malformed"));
    }
    ...
    {
        return Err(archive_error(
            "native archive local/central entry metadata is inconsistent",
        ));
    }
    ...
    let data_start = cursor
        ...
        .and_then(|value| value.checked_add(extra_len))
        .ok_or_else(|| archive_error("native archive local entry offset overflowed"))?;
    let data_end = data_start
        .checked_add(entry.compressed_size)
        .ok_or_else(|| archive_error("native archive data range overflowed"))?;
    if data_end > bytes.len() {
        return Err(archive_error("native archive entry data is truncated"));
    }

    let compressed = &bytes[data_start..data_end];
    let output = match entry.method {
        0 => compressed.to_vec(),
        8 => inflate_raw(compressed, entry.uncompressed_size, cancellation)?,
        _ => {
            return Err(archive_error(
                "native archive uses an unsupported compression method",
            ));
        }
    };
    ...
    if output.len() != entry.uncompressed_size {
        return Err(archive_error(
            "native archive entry decompressed to an unexpected size",
        ));
    }
...
fn u16_at(bytes: &[u8], offset: usize) -> Result<u16> {
    let data = bytes
        .get(offset..offset + 2)
        .ok_or_else(|| archive_error("native archive integer is out of bounds"))?;

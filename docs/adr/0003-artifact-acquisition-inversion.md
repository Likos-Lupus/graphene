# ADR-0003: Invert Artifact Acquisition at the Installer Boundary
...
Service composition owns the verified HTTP/download/cache pipeline, while domain installation must
reuse that pipeline without introducing the forbidden dependency
...
`graphene-service` owns the concrete adapter and delegates each acquisition to the existing verified
`ArtifactService`.
...
- integrity and cancellation behavior is reused rather than copied;
